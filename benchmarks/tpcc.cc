...
rc_t tpcc_worker::txn_query2() {
 ...
 for(uint warehouse_id=1; warehouse_id <= home_warehouse_id; warehouse_id++){
  ...
  for(uint customerDistrictID=1; customerDistrictID<NumDistrictsPerWarehouse(); customerDistrictID++){
    for(uint customerWarehouseID=1; customerWarehouseID<NumWarehouses(); customerWarehouseID++){
      ...
      for (auto &k : c_no.output) {
        ...
        const order_line::key k_ol_0(warehouse_id, districtID, k_no->no_o_id, 1);
        const order_line::key k_ol_1(warehouse_id, districtID, k_no->no_o_id, 15);
        TryCatch(tbl_order_line(warehouse_id)->Scan(txn, Encode(str(Size(k_ol_0)), k_ol_0), &Encode(str(Size(k_ol_1)), k_ol_1), c_ol));
        sum += c_ol.sum;
      }
    }
  }
